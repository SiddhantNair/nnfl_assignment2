...
utils = tf.keras.applications.imagenet_utils
...
def get_label(predictions):
    prediction = utils.decode_predictions(predictions)
...
def image_from_file(path):
    image = preprocessing.image.load_img(path, target_size=(299, 299))
    image = preprocessing.image.img_to_array(image)
    image = expand_dims(image, axis=0)
    image = utils.preprocess_input(image, mode='tf')
...
def InceptionV3():
    ...
    model.load_weights(f"{current_dir}/weights/imagenet_weights.h5")
